<thead>
  <tr>
    <th scope="col">Purchase Date</a></th>
    <th scope="col">info</th>
    <th scope="col">Who Payed</th>
    <th scope="col">For </th>
    <th scope="col">How Much</th>
  </tr>
</thead>
<tbody>
  <?php
  $servername = "localhost";
  $username = "root";
  $password = "";
  $dbname = "finance";

  // Create connection
  $conn = new mysqli($servername, $username, $password, $dbname);
  // Check connection
  if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
  }

  $orderby = " ORDER BY c.cash_Date DESC ";
  if (isset($_GET['order_by']) && isset($_GET['sort'])) {
    $orderby = ' order by ' . $_GET['order_by'] . ' ' . $_GET['sort'];
  }


  $sql = "SELECT c.cash_Date,c.cash_Value,c.cash_Descr, pt.type_Name, u.usr_Name 
FROM allcash c
RIGHT JOIN purchase_type pt ON c.type_Id = pt.type_Id
INNER JOIN users u ON c.usr_Id = u.usr_Id
WHERE pt.type_Id = '1' OR pt.type_Id = '2'" . $orderby . "";

  $result = $conn->query($sql);
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      echo "<tr>";
      echo "<td>" . $row['cash_Date'] . "</td>";
      echo "<td>" . $row['cash_Descr'] . "</td>";
      echo "<td>" . $row['usr_Name'] . "</td>";
      echo "<td>" . $row['type_Name'] . "</td>";
      echo "<td>" . $row['cash_Value'] . "</td>";
      echo "</tr>";
    }
  } else {
    echo "No data";
  };

  $SUMsql = "SELECT SUM(cash_Value) AS ValSum FROM allcash WHERE type_Id = '1' OR type_Id = '2'";
  $SUMresult = $conn->query($SUMsql);
  $row = mysqli_fetch_assoc($SUMresult);
  $sum = $row['ValSum'];

  echo "<tr>";
  echo "<td> </td>";
  echo "<td> </td>";
  echo "<td> </td>";
  echo "<td> </td>";
  echo "<td> <h5>" . $row['ValSum'] . "</h5></td>";
  echo "</tr>";

  $conn->close();
  ?>
</tbody>
